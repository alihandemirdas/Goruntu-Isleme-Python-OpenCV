...
while True:
    ...
    cv2.putText(frame, "Toplam Arac Sayisi : " + str(arac_sayisi), (350, 20), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.8,
                (0, 0, 0), 2)
    cv2.putText(frame, "Giren Arac Sayisi : " + str(giren_arac_sayisi), (350, 40), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.8,
                (0, 0, 255), 2)
    cv2.putText(frame, "Cikan Arac Sayisi : " + str(cikan_arac_sayisi), (350, 60), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.8,
                (34, 233, 29), 2)

    cv2.imshow("Cikti", frame)

    if cv2.waitKey(1) & 0xFF == ord("q"):  # q'ya Basıldığında Kapatılacak
        break
